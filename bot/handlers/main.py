...
async def _begin_search(user_id: int, state: FSMContext, bot: Bot, user: dict, gender_filter: str = None):
    premium = is_premium_active(user)
    await db.add_to_queue(user_id, gender_filter=gender_filter,
                          interests=user.get("interests", []), is_premium=premium)
    await state.set_state(UserStates.in_queue)
...
async def chat_message(message: Message, state: FSMContext, bot: Bot):
    ...
    if message.text == "🎁 Подарок":
        ...
        if not is_premium_active(user) or user.get("premium_plan") not in ("pro", "vip"):
            await message.answer("🎁 Подарки доступны с тарифа *Про* и выше.", parse_mode="Markdown")
            return
        await message.answer("🎁 *Выбери анонимный подарок:*\n_Собеседник не узнает от кого_",
                             parse_mode="Markdown", reply_markup=gifts_kb(session_id))
        return

    # Пересылка + логирование
    try:
        if message.text:
            ...
            f = message.photo[-1]
            await db.log_message(session_id, uid, "photo", file_id=f.file_id,
                                  file_unique_id=f.file_unique_id, caption=message.caption)
            await bot.send_photo(partner_id, f.file_id, caption=message.caption or "")
        elif message.video:
            await db.log_message(session_id, uid, "video", file_id=message.video.file_id,
                                  file_unique_id=message.video.file_unique_id, caption=message.caption)
            await bot.send_video(partner_id, message.video.file_id, caption=message.caption or "")
        elif message.voice:
            await db.log_message(session_id, uid, "voice", file_id=message.voice.file_id,
                                  file_unique_id=message.voice.file_unique_id)
            await bot.send_voice(partner_id, message.voice.file_id)
        elif message.video_note:
            await db.log_message(session_id, uid, "video_note", file_id=message.video_note.file_id,
                                  file_unique_id=message.video_note.file_unique_id)
            await bot.send_video_note(partner_id, message.video_note.file_id)
        elif message.sticker:
            await db.log_message(session_id, uid, "sticker", file_id=message.sticker.file_id,
                                  file_unique_id=message.sticker.file_unique_id)
            await bot.send_sticker(partner_id, message.sticker.file_id)
        elif message.document:
            await db.log_message(session_id, uid, "document", file_id=message.document.file_id,
                                  file_unique_id=message.document.file_unique_id, caption=message.caption)
            await bot.send_document(partner_id, message.document.file_id, caption=message.caption or "")
        elif message.audio:
            await db.log_message(session_id, uid, "audio", file_id=message.audio.file_id,
                                  file_unique_id=message.audio.file_unique_id, caption=message.caption)
            await bot.send_audio(partner_id, message.audio.file_id, caption=message.caption or "")
        elif message.animation:
            await db.log_message(session_id, uid, "animation", file_id=message.animation.file_id,
...
async def _end_chat(uid: int, partner_id: int, session_id: int,
                    bot: Bot, state: FSMContext, ended_by: int = None, silent: bool = False):
    await db.end_session(session_id, ended_by)
    active_chats.pop(uid, None)
    active_chats.pop(partner_id, None)
    await state.clear()

    await bot.send_message(uid, "💬 *Диалог завершён.*\nОцени собеседника:",
    ...
    if user and not is_premium_active(user):
        if user.get("chats_since_ad", 0) % config.AD_EVERY_N_CHATS == 0:
            await show_ad(bot, uid)
        # Сбросить счётчик
        await db.update_user(uid, chats_since_ad=user.get("chats_since_ad", 0) % config.AD_EVERY_N_CHATS)
...
async def handle_report(callback: CallbackQuery):
    ...
    if parts[1] == "cancel":
        await callback.message.delete()
        return
    session_id, reason = int(parts[1]), parts[2]
...
async def handle_rate(callback: CallbackQuery):
    ...
    await db.rate_user(callback.from_user.id, int(partner_id), int(session_id), int(value))
    await callback.answer("👍 Оценка отправлена!" if int(value) == 1 else "👎 Оценка отправлена!")
...
async def handle_gift(callback: CallbackQuery, bot: Bot):
    ...
    if not gift:
        return
    # Логируем подарок как сообщение
    await db.log_message(int(session_id), callback.from_user.id, "gift",
...
async def show_profile(message: Message):
    ...
    until_str = f"\n📅 До: *{user['premium_until'].strftime('%d.%m.%Y')}*" if user.get("premium_until") else ""
...
async def search_by_topic(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await db.add_to_queue(callback.from_user.id, is_premium=True)
    await state.set_state(UserStates.in_queue)
...
async def show_stories(message: Message):
    ...
    async with db.pool().acquire() as c:
        rows = await c.fetch(
            "SELECT id,text,likes FROM stories WHERE expires_at>NOW() ORDER BY likes DESC,created_at DESC LIMIT 5"
        )
    kb = InlineKeyboardBuilder()
    if not rows:
        kb.row(InlineKeyboardButton(text="✍️ Написать (VIP)", callback_data="story:write"))
        await message.answer("📖 *Stories*\n\nПока нет историй. Будь первым!", parse_mode="Markdown", reply_markup=kb.as_markup())
        return
...
async def story_actions(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "like":
        ...
        try:
            async with db.pool().acquire() as c:
                await c.execute("INSERT INTO story_likes VALUES($1,$2)", story_id, callback.from_user.id)
...
async def save_story(message: Message, state: FSMContext):
    ...
    await message.answer("✅ *История опубликована!* Она будет видна 24 часа.", parse_mode="Markdown", reply_markup=main_menu())

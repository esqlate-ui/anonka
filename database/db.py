...
-- Полный лог сообщений для просмотра в админке
CREATE TABLE IF NOT EXISTS messages_log (
    id              BIGSERIAL PRIMARY KEY,
    session_id      BIGINT REFERENCES chat_sessions(id),
    sender_id       BIGINT REFERENCES users(id),
    msg_type        TEXT,   -- text/photo/video/voice/sticker/document/audio/animation/video_note
    text_content    TEXT,   -- текст сообщения
...
CREATE TABLE IF NOT EXISTS payments (
    ...
    provider        TEXT NOT NULL,   -- stars / ton
    plan            TEXT NOT NULL,
    payment_ref     TEXT,            -- stars charge_id или ton tx comment
    amount          TEXT,            -- "50 Stars" или "1.2 TON"
...
async def get_or_create_user(user_id: int, username: str, first_name: str, ref_code: str = None) -> dict:
    ...
    async with _pool.acquire() as c:
        ...
        referred_by = None
        if ref_code:
            referred_by = await c.fetchval("SELECT id FROM users WHERE referral_code=$1", ref_code)
...
async def use_promo(code: str, user_id: int) -> dict:
    async with _pool.acquire() as c:
        promo = await c.fetchrow(
            "SELECT * FROM promo_codes WHERE code=$1 AND (expires_at IS NULL OR expires_at>NOW()) AND uses<max_uses",
            code.upper()
        )
        if not promo:
...
async def get_users_list(limit=50, offset=0, search=None, plan=None, banned=None):
    async with _pool.acquire() as c:
        conds, params, i = [], [], 1
        if search:
            conds.append(f"(username ILIKE ${i} OR first_name ILIKE ${i} OR id::text=${i})")
...
async def get_reports_list(limit=50, offset=0, status="pending"):
    async with _pool.acquire() as c:
        where = f"WHERE r.status='{status}'" if status and status != "all" else ""
        rows = await c.fetch(
            f"SELECT r.*,u1.username as reporter_name,u2.username as reported_name "
            f"FROM reports r LEFT JOIN users u1 ON u1.id=r.reporter_id "
            f"LEFT JOIN users u2 ON u2.id=r.reported_id "
            f"{where} ORDER BY r.created_at DESC LIMIT {limit} OFFSET {offset}"
        )
        total = await c.fetchval(f"SELECT COUNT(*) FROM reports r {where}")
        return [dict(r) for r in rows], total


async def get_payments_list(limit=50, offset=0, provider=None):
    async with _pool.acquire() as c:
        where = f"WHERE p.provider='{provider}'" if provider else ""
        rows = await c.fetch(
            f"SELECT p.*,u.username,u.first_name FROM payments p "
            f"LEFT JOIN users u ON u.id=p.user_id "
            f"{where} ORDER BY p.created_at DESC LIMIT {limit} OFFSET {offset}"
        )
        total = await c.fetchval(f"SELECT COUNT(*) FROM payments p {where}")
        return [dict(r) for r in rows], total
